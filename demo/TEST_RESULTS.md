...
数据范围：仅 `demo/synthetic-workspace/` 中的虚构资料

| 检查 | 结果 | 验证内容 |
| --- | --- | --- |
| 演示记录结构 | PASS | 5 个 JSON 记录符合 workspace 数据约定 |
...
```text
PASS: 9 checks — synthetic demo only; no user workspace read
```

这不是对真实招聘网站、真实公司资格或投递流程的测试；真实使用时，Skill 会重新核验当前年度的官方来源。
